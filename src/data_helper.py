...
def _train_transform_to_matrices(*args, channel_mode = 1):
    """
    ...
    if channel_mode == 1:
        ...
        return img_array, targets
    img = Image.open(file_path.replace('test//', 'test//rgb_'))
    img2 = Image.open(file_path.replace('test//', 'test//ind_'))
    img.thumbnail(img_resize)
    img2.thumbnail(img_resize)  # Resize the image

    # Augment the image `img` here

    # Convert to RGB and normalize
    img_array = np.asarray(img.convert("RGB"), dtype=np.float32) / 255
    img_array2 = np.asarray(img2.convert("RGB"), dtype=np.float32) / 255

    targets = np.zeros(len(labels_map))
    for t in tags.split(' '):
        targets[labels_map[t]] = 1
    return np.dstack((img_array, img_array2)), targets
...
def _test_transform_to_matrices(*args):
    ...
    img2 = Image.open('{}/{}'.format(test_set_folder, file_name.replace('rgb', 'ind')))
    img.thumbnail(img_resize)
    img2.thumbnail(img_resize)
    ...
    return np.dstack((img_array, img_array2)), file_name


def _get_train_matrices(train_set_folder, train_csv_file, img_resize, process_count, channel_mode=1):
    """
...
def _get_test_matrices(test_set_folder, img_resize, process_count):
    x_test = []
    x_test_filename = []
    files_name = glob.glob(test_set_folder + '/rgb*')
    files_name = [i.split('\\')[-1] for i in files_name]
...
def preprocess_train_data(train_set_folder, train_csv_file, img_resize=(32, 32), process_count=cpu_count(), channel_mode = 1):
    """
